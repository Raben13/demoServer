...
function call() {
  ...
  pc1 = new RTCPeerConnection(peerConnectionConfig, pc_constraints);
...
function gotMessageFromServer(message) {
    pc1.setRemoteDescription(new RTCSessionDescription({"type":message.type, "sdp":message.sdp})).then(
        function() {
        },
...
function onIceCandidate(pc, event) {
  trace(getName(pc) + ' ICE candidate: \n' + (event.candidate ?
    ...
    if(event.candidate == null) {
      ...
      sendMediabridgeRequest(pc.localDescription.sdp);
    }
}
...
socket.on('message', function (message) {
    var message = JSON.parse(message)
      gotMessageFromServer(message.type)
})
